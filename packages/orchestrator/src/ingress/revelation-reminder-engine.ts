...
import { selectAsmaReminderCandidates } from '@moonwitness/revelation';
...
async function choosePreviousScripture(seed: number): Promise<PreviousScriptureReference> {
  ...
  return {
    ...
    note: 'The current registry contains Islamic reference metadata, not a verified corpus of the original revealed text. No unverified Bible/Psalm/Torch passage is fabricated here.',
    sourceRefs: [book.id]
  };
}

...
export class PostgresProvider implements PersistenceStore {
  ...
  auditStore() {
    ...
    return {
      ...
      verify: async () => { const result = await store.query('SELECT * FROM audit_ledger ORDER BY timestamp,audit_id'); let previous=''; for(const row of result.rows){const record=normalizeAudit(row); if(row.previous_hash!==previous || hashAudit(record,previous)!==row.hash)return{valid:false,count:result.rows.length,head:previous||null}; previous=row.hash;} return{valid:true,count:result.rows.length,head:previous||null}; },
    };
  }

...
export const MIGRATIONS: Migration[] = [
  ...
  },
  {
    ...
    sqliteSql: `ALTER TABLE entities ADD COLUMN audit_json TEXT NOT NULL DEFAULT '{}'; ALTER TABLE relations ADD COLUMN audit_json TEXT NOT NULL DEFAULT '{}'; ALTER TABLE event_ledger ADD COLUMN audit_json TEXT NOT NULL DEFAULT '{}'; ALTER TABLE projections ADD COLUMN audit_json TEXT NOT NULL DEFAULT '{}'; ALTER TABLE evidence ADD COLUMN audit_json TEXT NOT NULL DEFAULT '{}'; CREATE TABLE IF NOT EXISTS audit_ledger (audit_id TEXT PRIMARY KEY, operation TEXT NOT NULL, model_type TEXT NOT NULL, record_id TEXT NOT NULL, actor_id TEXT NOT NULL, timestamp TEXT NOT NULL, changed_fields_json TEXT NOT NULL, before_json TEXT, after_json TEXT, previous_hash TEXT, hash TEXT NOT NULL, correlation_id TEXT, reason TEXT); CREATE INDEX IF NOT EXISTS idx_audit_record ON audit_ledger(record_id, timestamp);`,
  ...
  },
  {
    ...
    sqliteSql: `CREATE TABLE IF NOT EXISTS system_traces (request_id TEXT PRIMARY KEY, correlation_id TEXT NOT NULL, route TEXT NOT NULL, status_code INTEGER, duration_ms REAL, error_message TEXT, started_at TEXT NOT NULL, completed_at TEXT); CREATE TABLE IF NOT EXISTS system_jobs (id TEXT PRIMARY KEY, type TEXT NOT NULL, status TEXT NOT NULL, payload_json TEXT NOT NULL, result_json TEXT, error_message TEXT, created_at TEXT NOT NULL, updated_at TEXT NOT NULL);`,
  ...
  },
  {
    ...
    sqliteSql: `CREATE TABLE IF NOT EXISTS auth_users (user_id TEXT PRIMARY KEY, username TEXT UNIQUE NOT NULL, password_hash TEXT NOT NULL, rid TEXT, roles_json TEXT NOT NULL, active INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL); CREATE TABLE IF NOT EXISTS auth_revoked_tokens (token_hash TEXT PRIMARY KEY, revoked_at TEXT NOT NULL); CREATE TABLE IF NOT EXISTS idempotency_records (key TEXT PRIMARY KEY, request_hash TEXT NOT NULL, status_code INTEGER NOT NULL, body_json TEXT NOT NULL, created_at TEXT NOT NULL); CREATE INDEX IF NOT EXISTS idx_auth_users_username ON auth_users(username);`,
...

  {
    ...
    sqliteSql: `CREATE TABLE IF NOT EXISTS witness_nodes (hash TEXT PRIMARY KEY, node_id TEXT UNIQUE NOT NULL, kind TEXT NOT NULL, payload_json TEXT NOT NULL, parents_json TEXT NOT NULL, occurred_at TEXT NOT NULL, actor_id TEXT, nonce TEXT NOT NULL); CREATE INDEX IF NOT EXISTS idx_witness_nodes_occurred ON witness_nodes(occurred_at); CREATE TABLE IF NOT EXISTS witness_checkpoints (checkpoint_id TEXT NOT NULL, witness_id TEXT NOT NULL, root TEXT NOT NULL, heads_json TEXT NOT NULL, node_count INTEGER NOT NULL, created_at TEXT NOT NULL, algorithm TEXT NOT NULL, public_key TEXT NOT NULL, signature TEXT NOT NULL, PRIMARY KEY(checkpoint_id,witness_id)); CREATE INDEX IF NOT EXISTS idx_witness_checkpoint_root ON witness_checkpoints(root,created_at); CREATE TABLE IF NOT EXISTS witness_keys (key_id TEXT PRIMARY KEY, witness_id TEXT NOT NULL, algorithm TEXT NOT NULL, public_key TEXT NOT NULL, status TEXT NOT NULL, created_at TEXT NOT NULL, revoked_at TEXT, superseded_by TEXT); CREATE INDEX IF NOT EXISTS idx_witness_keys_witness ON witness_keys(witness_id,status);`,
...

  {
    ...
    sqliteSql: `CREATE INDEX IF NOT EXISTS idx_audit_timestamp ON audit_ledger(timestamp);`,
...

  {
    ...
    sqliteSql: `CREATE TABLE IF NOT EXISTS auth_sessions (session_id TEXT PRIMARY KEY, user_id TEXT NOT NULL, refresh_token_hash TEXT UNIQUE NOT NULL, created_at TEXT NOT NULL, refresh_expires_at TEXT NOT NULL, last_seen_at TEXT NOT NULL, revoked_at TEXT, rotation_counter INTEGER NOT NULL DEFAULT 1); CREATE INDEX IF NOT EXISTS idx_auth_sessions_user ON auth_sessions(user_id, revoked_at); CREATE INDEX IF NOT EXISTS idx_auth_sessions_refresh ON auth_sessions(refresh_token_hash);`,

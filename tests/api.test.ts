...
test('native HTTP API exposes the universal kernel', async () => {
  ...
  const body = await response.json() as { ok: boolean; kernel: boolean; ledger: boolean; environment: string; storageDriver: string; release: string; request_id?: string };
...
test('native HTTP API exposes deployment readiness', async () => {
  ...
  assert.equal(response.status, 200);
  const body = await response.json() as { status: string; release: string; storageDriver: string; request_id?: string };
...
test('native HTTP API serves AI analysis', async () => {
  ...
  assert.equal(response.status, 200);
  const body = await response.json() as { mizan?: unknown; semantic?: unknown; lifecycle?: unknown; request_id?: string };

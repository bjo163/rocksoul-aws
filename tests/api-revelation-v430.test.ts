...
test('v4.30 Revelation API exposes Asma, moral graph, event interpreter and moral lifecycle without legacy 99 route', async()=>{
  const dataDir=await fs.mkdtemp(path.join(os.tmpdir(),'mw-api-revelation-430-'));
  ...
  try {
    ...
    assert.equal(asma.body.protocol,'PURE_REVELATION_ASMA_V2');
    assert.equal(asma.body.invariants.canonical99Hardcoded,false);
    assert.ok(asma.body.candidateCount>100);

    const ontology=await get(base,'/api/v1/revelation/divine-ontology');
    assert.equal(ontology.response.status,200);
    assert.equal(ontology.body.protocol,'PURE_REVELATION_DIVINE_ONTOLOGY_V1');
    assert.ok(ontology.body.counts.concepts>100);
    assert.equal(ontology.body.invariants.canonicalDivineNamePromotedAutomatically,false);

    const graph=await get(base,'/api/v1/revelation/moral-graph');
    assert.equal(graph.response.status,200);
    assert.equal(graph.body.protocol,'REVELATION_MORAL_GRAPH_V2');
    ...
    const grammar=await get(base,'/api/v1/revelation/grammar');
    assert.equal(grammar.response.status,200);
    assert.equal(grammar.body.protocol,'REVELATION_GRAMMAR_ENGINE_V1');
    assert.equal(grammar.body.invariants.normativeAuthority,false);
    assert.equal(grammar.body.invariants.canonicalRootClaimed,false);
    assert.ok(grammar.body.frameCount>1000);

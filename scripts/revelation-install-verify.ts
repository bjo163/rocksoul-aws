...
const driver=(arg('driver',process.env.STORAGE_DRIVER??config.storage?.driver??'postgres') as 'postgres'|'sqlite'|'file'|'memory');
...
const store=createPersistence({driver,postgres,fileDir:process.env.PERSISTENCE_DIR,sqliteFile:process.env.SQLITE_FILE});
try {
  await store.ready?.();

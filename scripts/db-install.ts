...
if (driver === 'postgres') {
  ...
  const lockClient = new Client(postgres);
  await lockClient.connect();
  console.log('Waiting for exclusive database installation lock...');
  await lockClient.query('SELECT pg_advisory_lock($1)', [837462903]);
  try {
    await installDatabase();
  } finally {
    await lockClient.query('SELECT pg_advisory_unlock($1)', [837462903]).catch(() => undefined);

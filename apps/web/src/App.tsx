...
const copy = {
  id: {
    ...
    statusEyebrow: 'BASELINE SAAT INI', statusTitle: 'Baseline integrasi v4.32', statusBody: 'Semantic, Revelation, Witness, review-gate, web contract, API hermetic lanes, dan baseline PostgreSQL 18 telah memiliki bukti sertifikasi. Advanced concurrency, PITR, dan distributed deployment tetap diperlakukan sebagai pekerjaan lanjutan.',
    ...
    footer: 'Baseline riset & rekayasa · 2026', back: 'Kembali ke atas ↑',
  ...
  en: {
    ...
    statusEyebrow: 'CURRENT BASELINE', statusTitle: 'v4.32 integration baseline', statusBody: 'Semantic, Revelation, Witness, review-gate, web contract, hermetic API lanes, and the PostgreSQL 18 baseline have certification evidence. Advanced concurrency, PITR, and distributed deployment remain follow-up work.',
    boundaryEyebrow: 'IMPORTANT BOUNDARY', boundaryQuote: 'MoonWitness can expose reasoning and evidence trails. It does not know hearts, the unseen, acceptance of deeds, or Allah’s final judgement.', boundaryBody: 'A score is an engineering signal. A corpus pattern is an analytical finding. A human disposition is an operational decision. None may be promoted into Divine judgement.', footer: 'Research & engineering baseline · 2026', back: 'Back to top ↑',
...
export default function App() {
  ...
  return <main id="top"><a className="mw-skip" href="#content">{preferences.locale === 'id' ? 'Lewati ke konten utama' : 'Skip to main content'}</a>
    ...
    <div id="content" tabIndex={-1}>
      ...
      <section className="signal-strip" aria-label="Release status"><div className="shell strip-inner"><span>RELEASE <strong>4.32.0</strong></span><span>CORPUS <strong>18,328 passages</strong></span><span>QUR’AN <strong>6,236 ayat</strong></span><span>WITNESS <strong>HASH COMMITMENT</strong></span></div></section>
      ...
      <section className="section shell" id="status"><div className="status-card"><div><p className="eyebrow"><span /> {t.statusEyebrow}</p><h2>{t.statusTitle}</h2><p>{t.statusBody}</p></div><div className="status-list"><p><i className="ok" />Preflight & certification</p><p><i className="ok" />1,007 API checks</p><p><i className="ok" />500 adversarial Mīzān cases</p><p><i className="ok" />PostgreSQL 18 baseline</p></div></div></section>

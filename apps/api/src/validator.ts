...
export const v = {
  ...
  number: (): ValidatorFn<number> => (val) => typeof val === 'number' && !isNaN(val) ? { valid: true, value: val } : { valid: false, errors: ['Expected number'] },
  ...
  object: <T extends Record<string, ValidatorFn<any>>>(shape: T): ValidatorFn<{ [K in keyof T]: T[K] extends ValidatorFn<infer U> ? U : never }> => (val) => {
    ...
    for (const key of Object.keys(shape)) {
      ...
      if (res.valid) {
        if (res.value !== undefined) obj[key] = res.value;
      } else {
        res.errors.forEach(e => errs.push(`.${key}: ${e}`));
      }
    }
    return errs.length > 0 ? { valid: false, errors: errs } : { valid: true, value: obj };
  },
  any: (): ValidatorFn<any> => (val) => ({ valid: true, value: val }),
};
